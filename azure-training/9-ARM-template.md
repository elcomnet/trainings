# Lab 9 - Manage Azure resources by using Azure Resource Manager Templates
...
1. On the **Create a managed disk** page, configure the disk and then select **Ok**. 
    ...
    | Resource Group | `workshop-X` (If necessary, select **Create new**.)
    | Disk name | `student-X-disk1` | 
    | Region | **East US** |
...
1. Within the editor pane, make these changes.
    ...
    + Change **disks_student_disk1_name** to `disk_name` (two places to change)
    + Change **student-X-disk1** to `student-X-disk2` (one place to change)
...
1. Make this change so it matches the template file.
    ...
    Change **disks_student_disk1_name** to **disk_name** (one place to change)
...
1. Complete the custom deployment settings:
    ...
    | Disk_name | `student-X-disk2` |

1. Select **Review + Create** and then select **Create**.

1. Select **Go to resource**. Verify **student-X-disk2** was created.

1. On the **Overview** blade, select the resource group, **workshop-X**. You should now have two disks.
...
1. Make a change. For example, change the disk name to **student-X-disk3**. Use **Ctrl +S** to save your changes. 
...
1. To deploy to a resource group, use **New-AzResourceGroupDeployment**.
    ...
    ```powershell
    New-AzResourceGroupDeployment -ResourceGroupName workshop-X -TemplateFile template.json -TemplateParameterFile parameters.json
    ```
...
1. Make a change. For example, change the disk name to **student-X-disk4**. Use **Ctrl +S** to save your changes. 
...
1. To deploy to a resource group, use **az deployment group create**.
    ...
    ```sh
    az deployment group create --resource-group workshop-X --template-file template.json --parameters parameters.json
    ```
...
1. Locate and download the **azuredeploydisk.bicep** file.
...
1. Make the following changes:
    ...
    + Change the **managedDiskName** value to `student-X-Disk4`.
...
1. Now, deploy the template.
    ...
    ```
    az deployment group create --resource-group workshop-X --template-file azuredeploydisk.bicep
    ```
